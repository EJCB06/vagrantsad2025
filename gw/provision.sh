...
echo "########################################"
echo " Aprovisionando Gateway (ECUEBER) "
echo "########################################"
echo "-----------------"
...
export EASYRSA_REQ_ORG="El puche"
export EASYRSA_REQ_EMAIL="admin@ecueber.org"
export EASYRSA_REQ_OU="Departamento De Delincuencia Juvenil"
...
#####################################################
# Generamos las claves de servidor (CAMBIADO A SERVIDOR-ECUEBER)
/etc/openvpn/easy-rsa/easyrsa --batch  gen-req servidor-ecueber nopass
# Firmamos la clave pública del servidor
/etc/openvpn/easy-rsa/easyrsa --batch sign-req server servidor-ecueber
# Generamos clave TLS-Crypt
cd /etc/openvpn/server/
openvpn --genkey secret ta.key

# Copiamos claves de servidor y ca a /etc/openvpn
# OJO: Aquí usamos servidor-ecueber.crt y .key
cp /etc/openvpn/easy-rsa/pki/issued/servidor-ecueber.crt /etc/openvpn/server
cp /etc/openvpn/easy-rsa/pki/ca.crt /etc/openvpn/server
cp /etc/openvpn/easy-rsa/pki/private/servidor-ecueber.key /etc/openvpn/server
...
<LDAP>
    # IP de tu máquina idp (CAMBIADA A 172.2.2.2)
    URL             ldap://172.2.2.2
    # Usuario con el que OpenVPN busca en el directorio (DOMINIO ECUEBER)
    BindDN          "cn=admin,dc=ecueber,dc=org"
    Password        "${LDAP_PASS}"
    Timeout         15
</LDAP>

<Authorization>
    # Base donde están tus usuarios (DOMINIO ECUEBER)
    BaseDN          "ou=ou_usuarios,dc=ecueber,dc=org"
    # Filtro de búsqueda del usuario que intenta conectar
    SearchFilter    "(&(uid=%u)(objectClass=posixAccount))"

    # RESTRICCIÓN POR GRUPO
    <Group>
        BaseDN          "ou=ou_grupos,dc=ecueber,dc=org"
